...
[a_ufloat, b_ufloat] = odr_params(days_unp, weights_unp, weight_gain_model, initial_params=[86, 0.99])
...
elif b_ufloat.n < target_b_min:
    print("You are gaining weight too slow!")
elif b_ufloat.n < target_b_max:
    print("You are on track!")
else:
    print("You are gaining weight too fast!")
...
print(frmtr.format("Estimated weight gained = {} kg", -a_ufloat*(1-b_ufloat**((max(days)-1)/7))))
...
plt.plot(days, weight_gain_model(days, *unp.nominal_values([a_ufloat, b_ufloat])), label='Fitted Curve', color='red')
plt.fill_between(days_plus_seven,  
                 weight_gain_model(days_plus_seven, a_ufloat.nominal_value - a_ufloat.std_dev, b_ufloat.nominal_value - b_ufloat.std_dev), 
                 weight_gain_model(days_plus_seven, a_ufloat.nominal_value + a_ufloat.std_dev, b_ufloat.nominal_value + b_ufloat.std_dev), 
                 color='red', alpha=0.3, label='Uncertainty')

# Plot target weight loss lines
plt.fill_between(days_plus_seven,  
                 weight_gain_model(days_plus_seven, a_ufloat.nominal_value - a_ufloat.std_dev, target_b_min), 
                 weight_gain_model(days_plus_seven, a_ufloat.nominal_value + a_ufloat.std_dev, target_b_max), 
                 color='blue', alpha=0.3, label='Target Range') 

plt.xlabel('Days')
plt.ylabel('Weight (kg)')
plt.title('Bulking Journey')
plt.legend()
plt.grid(True)
...
target_min_weight = round(weekly_medians[-1] * target_b_max, 1)
target_max_weight = round(weekly_medians[-1] * target_b_min, 1)
